...
export default async function BahasaDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5]">
      {/* Top bar */}
      <header className="bg-white/90 backdrop-blur-md border-b border-earth-300/50 px-4 py-3" role="banner">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <Link href="/" className="font-black text-[#1a1209] text-sm tracking-tight hover:text-amber-700 transition-colors duration-200 cursor-pointer" aria-label="Back to home">
            Nusantara<span className="text-amber-700">Basa</span>
          </Link>
          <Link href="/explore/map" className="inline-flex items-center gap-1 text-sm text-earth-600 hover:text-earth-700 transition-colors duration-200 cursor-pointer" aria-label="Back to map">
            <ArrowLeft />
            Kembali ke peta
          </Link>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 md:py-8" role="main">
        {/* Header */}
        <div className="mb-6 md:mb-8">
          <div className="flex items-start gap-3 md:gap-4">
            <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center text-white text-xl md:text-2xl font-bold flex-shrink-0" style={{ backgroundColor: rumpunColor }} aria-hidden="true">
            ...
            <div className="flex-1 min-w-0">
              ...
              {bahasa.nama_lokal && <p className="text-base md:text-lg text-earth-600 italic mt-0.5">{bahasa.nama_lokal}</p>}
            </div>
            {bahasa.status_vitalitas && (
              <span className="flex-shrink-0 inline-flex items-center gap-1.5 rounded-full text-xs px-3 py-1.5 font-medium text-white" style={{ backgroundColor: vitalitasColor }} role="status" aria-label={`Vitality status: ${bahasa.status_vitalitas}`}>
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-6 md:mb-8">
          <div className="bg-white rounded-2xl p-4 md:p-5 border border-earth-300/50 shadow-sm">
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="font-bold text-[#1a1209]">{penuturFormatted}</dd>
              </div>
            </dl>
          </div>
          <div className="bg-white rounded-2xl p-4 md:p-5 border border-earth-300/50 shadow-sm">
            ...
            {bahasa.fitur_linguistik ? (
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="text-[#1a1209]">{bahasa.fitur_linguistik.sistem_tulisan ?? "—"}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-earth-600">Tipe Morfologi</dt>
                  <dd className="text-[#1a1209]">{bahasa.fitur_linguistik.tipe_morfologi ?? "—"}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-earth-600">Urutan Kata</dt>
                  <dd className="font-mono text-[#1a1209] font-medium">{bahasa.fitur_linguistik.urutan_kata ?? "—"}</dd>
                </div>
              </dl>
        ...
        {bahasa.lokasi?.length > 0 && (
          <div className="bg-white rounded-2xl p-4 md:p-5 border border-earth-300/50 shadow-sm mb-6 md:mb-8">
            <h2 className="font-bold text-[#1a1209] mb-3 flex items-center gap-2">
              <MapPinIcon />
              Lokasi Sebaran
            </h2>
            <div className="flex flex-wrap gap-2" role="list" aria-label="Language distribution areas">
              {bahasa.lokasi.map((loc: any, i: number) => (
                <span key={i} className="px-3 py-1.5 bg-earth-100 text-earth-700 rounded-full text-sm font-medium" role="listitem">{loc.provinsi}{loc.kabupaten && `, ${loc.kabupaten}`}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {bahasa.auto_summary && (
          <div className="bg-white rounded-2xl p-4 md:p-5 border border-earth-300/50 shadow-sm mb-6 md:mb-8">
            ...
            <div className="text-sm text-earth-600 leading-relaxed">
        ...
        {bahasa.kosakata?.length > 0 && (
          <div className="bg-white rounded-2xl p-4 md:p-5 border border-earth-300/50 shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm" role="table" aria-label="Sample vocabulary">
                <thead>
                  <tr className="border-b border-earth-300">
                    <th className="text-left py-2 text-earth-600 font-medium" scope="col">Kata</th>
                    <th className="text-left py-2 text-earth-600 font-medium" scope="col">Arti</th>
                    <th className="text-left py-2 text-earth-600 font-medium" scope="col">Fonetik (IPA)</th>
                  </tr>
                </thead>
                <tbody>
                  {bahasa.kosakata.map((k: any, i: number) => (
                    <tr key={i} className="border-b border-earth-200">
                      <td className="py-2.5 font-medium text-[#1a1209]">{k.kata}</td>
                      <td className="py-2.5 text-earth-600">{k.arti_indonesia}</td>
                      <td className="py-2.5 font-mono text-earth-600">{k.fonetik_ipa ?? "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
