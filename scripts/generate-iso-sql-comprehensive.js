...
// Read comprehensive ISO mapping
const isoMapping = JSON.parse(fs.readFileSync('data/iso-mapping-comprehensive.json', 'utf8'));

// Generate SQL UPDATE statements with conflict handling
let sql = `-- Comprehensive ISO 639-3 codes update for Indonesian languages
-- Generated on ${new Date().toISOString().split('T')[0]}
-- Source: Ethnologue, Glottolog, ISO 639-3 registry
-- Total mappings: ${Object.keys(isoMapping).length}
...
fs.writeFileSync('scripts/enrich-iso-codes-comprehensive.sql', sql);
console.log(`Generated ${count} UPDATE statements`);
console.log('SQL file saved to scripts/enrich-iso-codes-comprehensive.sql');
